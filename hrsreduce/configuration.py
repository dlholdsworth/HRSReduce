...
if int(version('jsonschema')[0]) < 3:  # pragma: no cover
    logger.warning(
...
def get_configuration_for_instrument(**kwargs):
    local = dirname(__file__)
    instrument = "HRS"
    if instrument in ["hrsreduce", None]:
        fname = join(local, "settings", f"settings_hrsreduce.json")
    else:
...
def read_config(fname="settings_hrsreduce.json"):
