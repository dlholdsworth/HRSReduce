...
class CCF():
    ...
    def do_ccf_from_spec_wavelengths_and_widths(self,wave, intmtx, wa, wi, numbins):
        ...
        # Cross-correlation
        ccf_full = np.correlate(1-logspecint, ldelfn_array, mode='full') / len(ldelfn_array) #1 - logspecint
        ccf = ccf_full[center - numbins // 2 : center + numbins // 2 + 1]
        CCF.append(ccf)

        # Deconvolution step
        tresult = self.testing_deconvolution(ccf, dfn_auto)
        tresult =  1-tresult # 1 - tresult
    ...
    def execute(self):
        ...
        with fits.open(self.frame) as hdu:
            ...
            for ord in range(nord):
                ...
                wave = hdu[ext].data['Wave']
                flux = hdu[ext].data['Flux']/hdu[ext].data['Blaze']
                
                flux = np.nan_to_num(flux,nan=1.2)
                
                ii = np.where(np.logical_and(wa > np.min(wave)-10, wa < np.max(wave)+10))[0]
                wa_cut = wa[ii]
                wi_cut = wi[ii]
...
#                plt.show()
                ...
                plt.plot(ord,result.params['cen'].value+BVC,'o')

            RV_mean , summedweights = np.average(RV,weights = 1./np.array(RV_err),returned=True)
            RV_outerr = np.sqrt(1/summedweights)

            plt.title(str(RV_mean)+"±"+str(RV_outerr))
