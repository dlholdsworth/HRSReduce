...
class MasterLFC():
    ...
    def create_masterarc(self):
        ...
        #Test if Master Arc exists
        if not self.super:
            master_file = glob.glob(self.out_dir+self.mode+"_Master_LFC_"+self.sarm+str(yyyymmdd)+".fits")
        else:
            master_file = glob.glob(self.out_dir+self.mode+"_"+self.sarm+str(yyyymmdd)+".fits")
        ...
        if len(master_file) == 0:
            ...
            n=len(Arc_files)
            if n <1:
                logger.error("\n   !!! No LFC files found in {}. Check arm ({}) and night ({}). Exiting.\n".format(self.out_dir,self.arm,yyyymmdd))
                exit()
            Arc_concat = []
            Arc_files_short = []
            exptime = []
            ...
            for file in Arc_files:
                ...
                with fits.open(file) as hdu:
                    ...
                    if np.nanstd(hdu[0].data) > self.reject_limit:
                        ...
                        logger.warning("LFC file {} found not to have enough range ({}). Rejecting it.".format(file,int(np.nanstd(hdu[0].data))))
                        #Change the file name to show it is bad
                        ngt = os.path.basename(file)
                        yr = ngt[-17:-13]
                        ngt = ngt[-13:-9]
                        bad_dir = (self.base_dir+self.arm+"/"+yr+"/"+ngt+"/reduced/")
                        os.rename(file, str(bad_dir+"Bad_LFC_"+os.path.basename(file)))
            ...
            cnt_hdu = fits.ImageHDU(data=cnt.astype(np.int32),name="CNT")
            cnt_hdu.header.insert(8,('COMMENT',"Count of LFC frames used per pixel to calculate MasterLFC"))
            unc_hdu = fits.ImageHDU(data=arc_unc.astype(np.float32),name="UNC")
            unc_hdu.header.insert(8,('COMMENT',"Uncertainty of MasterLFC"))
            
            hdul = fits.HDUList([new_hdu, cnt_hdu, unc_hdu])
            
            if self.super:
                try:
                    super_dir = self.base_dir+self.arm+"/"+str(yyyymmdd[0:4]+"/Super_LFCs/")
                    os.mkdir(super_dir)
                except Exception:
                    pass
                master_file = str(super_dir)+self.mode+"_Super_LFC_"+str(self.sarm)+str(yyyymmdd)+".fits"
            else:
                master_file = str(self.out_dir)+self.mode+"_Master_LFC_"+str(self.sarm)+str(yyyymmdd)+".fits"
            hdul.writeto(master_file,overwrite=True)
        
        else:
            logger.info("Reading Master LFC frame "+master_file[0]+"\n")
            master_file = master_file[0]
            
        if self.plot:  # pragma: no cover
            with fits.open(master_file) as hdu:
                master_arc = hdu[0].data
            title = "Master LFC Frame"
            plt.title(title)
            plt.xlabel("x [pixel]")
            plt.ylabel("y [pixel]")
            bot, top = np.percentile(master_arc, (1, 99))
            plt.imshow(master_arc, vmin=bot, vmax=top, origin="lower")
            plt.savefig(self.out_dir+self.mode+"_Master_LFC_Frame.png",bbox_inches='tight',dpi=600)
            plt.close()
