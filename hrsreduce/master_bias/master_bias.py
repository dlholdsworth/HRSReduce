...
class MasterBias():
    ...
    def create_masterbias(self):
        ...
        if len(master_file) == 0:
            ...
            if len(Bias_files) <1:
                logger.error ("No correct Bias files found in night {} for arm {}. Check night and arm.".format(self.night,self.arm))
                logger.info ("Looking for the nearest Bias files and creating a Master Bias from them.")
                files = {}
                b_night = {}
                #Now search to find a night with the files
                m='HS' #Dummy for Bias as it is not used
                base_dir = self.in_dir[:-18]
                files['bias'],b_night['bias']= FindNearestFiles("Bias",self.night,m,base_dir,self.arm,logger)
                #Create the output directory if it does not exist
                self.out_dir = self.arm+"/"+b_night['bias'][0:4]+"/"+b_night['bias'][4:8]+"/reduced/"
                self.in_dir = self.arm+"/"+b_night['bias'][0:4]+"/"+b_night['bias'][4:8]+"/raw/"
                
                try:
                    os.mkdir(str(base_dir+self.out_dir))
                except Exception:
                    pass
                #Now apply the L0 Corrections to these Bias files
                files = L0Corrections(files,b_night,self.night,self.in_dir,self.out_dir,base_dir,self.sarm).run()
                self.out_dir = base_dir+self.arm+"/"+self.night[0:4]+"/"+self.night[4:8]+"/reduced/"
                Bias_files = files['bias']
