...
wave_file = '/Users/daniel/Desktop/SALT_HRS_DATA/Blu/2022/0717/reduced/bgoH202207170027.fits'
...
output = np.array([known_waveobs,known_spec,np.sqrt(known_spec)])
np.savetxt('thar_best.txt',output.T)
...
with fits.open(ref_file) as hdu:
    for i in range(0,42):
        ...
        plt.plot(Wave_P[i],FIBRE_P*10000)
        
        output = np.array([Wave_P[i],FIBRE_P*10000,FIBRE_P])

        np.savetxt('ThAr_order'+str(i)+'.txt',output.T)
