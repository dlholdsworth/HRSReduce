...
class Instrument:
    ...
    def get_expected_values(self, night, *args, **kwargs):
        expectations = {
            "bias": {
    ...
    def apply_filters(self, files, expected, input_dir, allow_calibration_only=False):
        """
        ...
        input_dir : str
            directory of the initial night in question
        ...
        # Use the header information determined and populate filters
            ...
            for step, step_data in result.items():
                ...
                if len(f[step]) == 0:
                    if step not in self.find_closest:
                        ...
                        print("Searching other directories",night+ timedelta(days=-1),input_dir)
                        #n1=night[0:4]+night[5:2]+night[7+2]
                        #print(n1)
                        j = None
    ...
    def find_nearest_files(self,type,night,m,base_dir,arm_colour,**kwargs):
        ...
        while not files:
            ...
            result = self.sort_files_2(prev_data_location,mode=m,**kwargs)
            ...
            result = self.sort_files_2(next_data_location,mode=m,**kwargs)
            files = result[idx]
            if m =="HS" and idx == 1 and len(files)<9:
                files = []
            else:
    ...
    def sort_files(
        self, input_dir, night, *args, allow_calibration_only=False, **kwargs
    ):
        """
        Sort a set of fits files into different categories
        types are: bias, flat, wavecal, orderdef, spec
        ...
        input_dir = input_dir.format(
            **kwargs, night=night, instrument=self.name
        )
        files = self.find_files(input_dir)
        ev = self.get_expected_values(night, *args, **kwargs)
        files = self.apply_filters(
            files, ev, input_dir, allow_calibration_only=allow_calibration_only
        )
        return files
...
class InstrumentWithModes(Instrument):
    ...
    def get_expected_values(self, night, mode):
        expectations = super().get_expected_values(night, mode)
