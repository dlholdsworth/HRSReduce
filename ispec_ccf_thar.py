...
def determine_radial_velocity_with_template(star_spectrum,template,model = '2nd order polynomial + gaussian fit'):
    ...
    models, ccf = cross_correlate_with_template(star_spectrum, template, \
                            ...
                            velocity_step=0.10, model = model, fourier=False)
...
if __name__ == '__main__':
    ...
    files =sorted(glob.glob("/Users/daniel/Desktop/SALT_HRS_DATA/Blu/2022/0711/reduced/bgoH202207110027.fits"))
    mask_file = "./hrsreduce/wave_cal/thar_best.txt"
    
    line_list = np.load("hrsreduce/wave_cal/HR_H_linelist_O.npy",allow_pickle=True).item()
    
    with fits.open('./hrsreduce/wave_cal/thar_best.fits') as hdul:
        header = hdul[0].header
        known_spec = (hdul[0].data)
        specaxis = str(1)
        known_flux = known_spec
        ...
        known_waveobs = xconv(np.arange(len(known_flux)))
    
    min_mask, max_mask = np.min(known_waveobs),np.max(known_waveobs)
    for file in files:
        with fits.open(file) as hdu:
            bjd=float(hdu[0].header['JD'])
            baryrv = 0
            known_rv = 0
            data= hdu['FIBRE_O'].data
            lam = hdu['WAVE_O'].data
            ...
            for ord in range(42):
                ...
                star_spectrum = np.recarray((len(lam[ord]), ), dtype=[('waveobs', float),('flux', float),('err', float)])
                star_spectrum['waveobs'] = lam[ord]
                star_spectrum['flux'] = data[ord] / np.nanmax(data[ord])
                star_spectrum['flux'] = np.nan_to_num(star_spectrum['flux'],nan=0.,posinf=0., neginf=0.)
                star_spectrum['err'] = np.sqrt(data[ord]- np.min(data[ord]))
                
                ii=np.where(np.logical_and(known_waveobs>np.min(star_spectrum['waveobs'])-1,known_waveobs<np.max(star_spectrum['waveobs'])+1))[0]
                template= np.recarray((len(known_waveobs[ii]), ), dtype=[('waveobs', float),('flux', float),('err', float)])
                template['waveobs'] = known_waveobs[ii]
                template['err'] = np.sqrt(known_flux[ii])
                
                
                template['flux'] = known_flux[ii] / np.max(known_flux[ii])
#                plt.plot(template['waveobs'], template['flux'])
##                plt.plot(star_spectrum['waveobs'],data[ord] / np.max(data[ord]),'g')
#                plt.plot(star_spectrum['waveobs'],star_spectrum['flux']/np.max(star_spectrum['flux']),'r')
#                plt.vlines(line_list[ord]['known_wavelengths_air'],0,0.5,'g')
#                plt.title(str(ord))
#                plt.show()
                ...
                plt.errorbar(np.min(lam[ord]),rv+baryrv-known_rv,rv_err)
                plt.plot(np.min(lam[ord]),rv,'o')


            rv = np.array(rvs)
            rv_err = np.array(rv_errs)
...
#                print(rv+baryrv-known_rv,rv_err)
    ...
    files =sorted(glob.glob("./MIDAS_Reductions/20220711/mbgphH202207110027_1wm.fits"))
    mask_file = "./hrsreduce/wave_cal/thar_best.txt"
    ...
    min_mask, max_mask = np.min(known_waveobs),np.max(known_waveobs)
    for file in files:
        with fits.open(file) as hdua:
            header = hdua[0].header
            spec_AK = (hdua[0].data)
            specaxis = str(1)
            known_flux = spec_AK
            wave_step = header['CDELT%s' % (specaxis)]
            ...
            reference_pixel = header['CRPIX%s' % (specaxis)]
            xconv = lambda v: ((v-reference_pixel+1)*wave_step+wave_base)
            waveobs_AK = xconv(np.arange(len(spec_AK)))
            ...
            star_spectrum = np.recarray((len(waveobs_AK), ), dtype=[('waveobs', float),('flux', float),('err', float)])
            star_spectrum['waveobs'] = waveobs_AK
            star_spectrum['flux'] = spec_AK
            star_spectrum['err'] = np.sqrt(spec_AK)
